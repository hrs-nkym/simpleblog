<template>
  <article class="container" v-if="post">
    <header>
      <nav id="back">
        <a href="/" title="前ページへ戻る"><img src="@/assets/back.png" /></a>
      </nav>
      <p class="post-category" :style="{ color: post.category.color }">
        {{ post.category.name }}
      </p>
    ...
    <hr class="divider" />
    <nav id="top">
      <a @click="scrollTop" title="一番上まで戻る"
        ><img src="@/assets/ue.png"
      /></a>
    </nav>
  </article>
</template>
    ...
    this.$http(`${this.$httpPosts}${this.id}/`, { credentials: "include" })
      ...
      .then((data) => {
        ...
        document
          ...
          .setAttribute("content", data.lead_text);
